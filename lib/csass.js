...
module.exports = async function (config) {
   ...
   if (Array.isArray(res) && res.length > 0) {
    for (let j = 0; j < res.length; j++) {
     ...
     if (fstat.isDirectory()) {
      if (includes.find(item => fpath.indexOf(item) > -1) && !ignores.find(item => fpath.indexOf(item) > -1)) {
 ...
 if (item.hot) {
  watchFiles(origin_path, {
   ...
   if (filePath && includes.find(item => filePath.indexOf(item) > -1) && !ignores.find(item => filePath.indexOf(item) > -1)) {
    const filename = path.parse(filePath).name + path.parse(filePath).ext
    await compileSassFile(filePath, filename, target_path)
   }
  })
